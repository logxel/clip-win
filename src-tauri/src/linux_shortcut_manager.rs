...
fn get_command_path() -> &'static str {
    ...
    if Utils::command_exists("clip-win") {
        return "clip-win";
    }
...
const SHORTCUTS: &[ShortcutConfig] = &[
    ShortcutConfig {
        ...
        command: "clip-win", // Will be replaced at runtime
        args: "",
        ...
        let modified = Utils::modify_file_atomic(&path, |content| {
            ...
            for line in lines.iter_mut() {
                ...
                if trimmed.starts_with("bindsym") && has_mod_v_binding(&trimmed) {
                    *line = format!("# {} # Commented by clip-win", line);
                    had_existing = true;
                }
            }

            // Add our binding at the end
            lines.push("\n# Clipboard History (added by clip-win)".to_string());
    ...
    fn unregister(&self, s: &ShortcutConfig) -> Result<()> {
        ...
        let modified = Utils::modify_file_atomic(&path, |content| {
            ...
            for line in lines {
                ...
                if line.contains("# Clipboard History (added by clip-win)") {
                    skip_comment = true;
                    continue;
                }
                ...
                if line.contains("# Commented by clip-win") {
                    let restored = line
                        .replace("# ", "")
                        .replace(" # Commented by clip-win", "");
